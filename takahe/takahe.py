    ...
    def pre_process_sentences(self):
        ...
        for i in range(self.length):
            ...
            for w in sentence:
                ...
                container.append((token.lower(), POS, token))
    ...
    def build_graph(self):
        ...
        for i in range(self.length):
            ...
            for j in range(sentence_len):
                ...
                token, POS, tokenUpper = self.sentence[i][j]
                # token = temp[0]
                # POS = temp[1]
                # tokenUpper = temp[2]
                # print(temp)
            ...
            for j in range(sentence_len):
                ...
                if mapping[j] == 0:
                    ...
                    prev_token, prev_POS, _ = self.sentence[i][j-1]
                    next_token, next_POS, _ = self.sentence[i][j+1]
    ...
    def build_graph(self, window=0):
        ...
        for i in range(len(self.sentences)):
            ...
            for j in range(len(sentence)):
                ...
                word, pos, wordUpper = self.wordpos_to_tuple(sentence[j])

                # Replace word/POS by (word, POS) tuple in the sentence
                sentence[j] = (word.lower(), pos, wordUpper)

                # Modify the POS tags of stopwords to exclude them
                if sentence[j][0] in self.stopwords:
                    sentence[j] = (sentence[j][0], "STOPWORD", sentence[j][2])
